...
void MyArray<T>::Add(T el)
{
	...
	else
	{
		if (extra == 0)
		{
			...
			for (int i = count; i < count + grow; i++)
			{
				tmp[i] = 0;
			}
			extra = grow;
...
void MyArray<T>::Append(const MyArray<T>& right)
{
	...
	for (int i = this->count, j = 0; i < tmp.count; i++, j++)
		tmp.arr[i] = right.arr[j];
	for (int i = tmp.count; i < this->extra + right.extra; i++)
		tmp.arr[i] = 0;
...
void MyArray<T>::InsertAt(T* elArr, const int SIZE, int pos)
{
	if (extra >= SIZE)
	{
		...
		for (int i = pos + SIZE, j = pos; i < count + SIZE + extra; i++, j++)
			tmp[i] = arr[j];
...
{
	if (posBegin < posEnd 
		&& posBegin >= 0 && posBegin < count 
		&& posEnd >= 0 && posEnd < count)
	{
		for (int i = 0; i <= posEnd - posBegin; i++)
		{
			RemoveAt(posBegin);
		}
